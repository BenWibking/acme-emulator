...
def search(f,
	...
	if d % 2 == 0:
		...
		v1 = 2 * (4 * np.pi)**((d - 1) / 2) * np.math.factorial(
			(d - 1) / 2) / np.math.factorial(d)
	...
	for i in range(m // batch):
		...
		if d > 1:
			...
			cloud = population[
				population[:, -1].argsort()][0:int(nrand * nrand_frac), 0:-1]
		...
		for j in range(batch):
			...
			cons = [{
				...
				'ineq',
				'fun':
				lambda x, localk=k: np.linalg.norm(
		...
		with executor() as e:
			points[n + batch * i:n + batch * (i + 1), -1] = list(
				e.map(
					f,
					list(
						map(cubetobox, points[n + batch * i:n + batch *
											(i + 1), 0:-1])))) / fmax
	...
	return points[0, 0:-1]	# best-fit parameters
