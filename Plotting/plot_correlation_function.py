...
def plot_2pcf(input_files,logy=True,title=None,ylabel=None,residuals=True,
			residuals_only=False, linear_format_xaxis=True,ymin=None,ymax=None,
			large_labels=False):
		color_cycle = ['black', 'blue', 'green', 'red', 'brown']
		style_cycle = ['-', '--', '-.', ':', '-..']

		if large_labels==True:
				axisfontsize=20
				legendfontsize=14
...
#                resid_max = 0.2
				...
				ax_resid.set_xlabel(r'$r_p$ ($h^{-1}$ Mpc)',fontsize=axisfontsize)
		elif residuals_only:
				axes = [ax_resid]
#                resid_max = max(np.abs(1.0-global_fmin), np.abs(1.0-global_fmax))
				resid_max = 0.2
		...
		for x in axes:
				...
				x.xaxis.set_tick_params(top=True,direction='in',which='both',
										labelsize=ticklabelfontsize)
				x.yaxis.set_tick_params(right=True,direction='in',which='both',
										labelsize=ticklabelfontsize)
...
parser.add_argument('--large_labels',default=False,action='store_true')
...
#                        ylabel=r'$\xi_{gg}$')
		plot_2pcf(args.input_file, title=args.figure_title, ylabel=args.figure_yaxis,
				...
				ymin=args.ymin, ymax=args.ymax, large_labels=args.large_labels)
		pdf.savefig()
