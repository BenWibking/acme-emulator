...
all_varied_parameters = ['ngal','siglogM','M0_over_M1','M1_over_Mmin','alpha','q_env','del_gamma','Omega_M','sigma_8', 'f_cen', 'A_conc', 'delta_b', 'delta_c']
all_varied_parameters_centralsonly = ['ncen','siglogM','q_env','Omega_M','sigma_8'] # *don't* use ngal here

use_log_parameter = ['ngal','ncen','siglogM','M1_over_Mmin','alpha','H0','sigma_8','M0_over_M1','Omega_M','f_cen','A_conc']
...
def task_compute_DeltaSigma_baldauf_centralsonly():
    ...
    for param_file in param_files_centralsonly:
        correlation_file = observable_path(txt_reconstructed_DeltaSigma_this_param_centralsonly(param_file))
        script = "./Analysis/compute_baldauf_estimator.py"
        deps = [correlation_file, script]
...
def task_compute_log_wp_centralsonly():
    ...
    for param_file in param_files_centralsonly:
        correlation_file = observable_path(txt_reconstructed_wp_this_param_centralsonly(param_file))
        script = "./Analysis/compute_ln_function.py"
        deps = [correlation_file, script]
...
def task_compute_log_DeltaSigma_baldauf_centralsonly():
    ...
    for param_file in param_files_centralsonly:
        correlation_file = observable_path(txt_reconstructed_DeltaSigma_baldauf_this_param_centralsonly(param_file))
        script = "./Analysis/compute_ln_function.py"
        deps = [correlation_file, script]
...
def task_compute_log_number_density_centralsonly():
    ...
    for param_file in param_files_centralsonly:
        correlation_file = observable_path(txt_galaxy_number_density_this_param_centralsonly(param_file))
        script = "./Analysis/compute_ln_array.py"
        deps = [correlation_file, script]
...
def task_compute_derivative_log_wp():
    ...
    for param_files, param in param_files_list:
        ...
        output = './Derivatives/ln_wp_%s.txt' % param
        targets = [output]
...
def task_compute_derivative_log_DeltaSigma():
    ...
    for param_files, param in param_files_list:
        ...
        script = "./Analysis/compute_partial_derivative.py"
        deps = correlation_files+[script]
        output = './Derivatives/ln_DeltaSigma_%s.txt' % param
        targets = [output]
...
def task_compute_derivative_log_DeltaSigma_baldauf():
    ...
    for param_files, param in param_files_list:
        ...
        script = "./Analysis/compute_partial_derivative.py"
        deps = correlation_files+[script]
        output = './Derivatives/ln_DeltaSigma_baldauf_%s.txt' % param
        targets = [output]
...
def task_compute_derivative_log_DeltaSigma_baldauf_centralsonly():
    ...
    for param_files, param in param_files_list_centralsonly:
        ...
        script = "./Analysis/compute_partial_derivative.py"
        deps = correlation_files+[script]
        output = './Derivatives/ln_DeltaSigma_baldauf_%s.centralsonly.txt' % param
        targets = [output]
...
def task_compute_derivative_log_ngal():
    ...
    for param_files, param in param_files_list:
        ...
        deps = correlation_files+[script]
        output = './Derivatives/ln_ngal_%s.txt' % param
        targets = [output]
...
def task_compute_derivative_log_wp_centralsonly():
    ...
    for param_files, param in param_files_list_centralsonly:
        ...
        script = "./Analysis/compute_partial_derivative.py"
        deps = correlation_files+[script]
        output = './Derivatives/ln_wp_%s.centralsonly.txt' % param
        targets = [output]
...
def task_compute_derivative_log_DeltaSigma_centralsonly():
    ...
    for param_files, param in param_files_list_centralsonly:
        ...
        script = "./Analysis/compute_partial_derivative.py"
        deps = correlation_files+[script]
        output = './Derivatives/ln_DeltaSigma_%s.centralsonly.txt' % param
        targets = [output]
...
def task_compute_derivative_log_ngal_centralsonly():
    ...
    for param_files, param in param_files_list_centralsonly:
        ...
        script = "./Analysis/compute_partial_derivative_array.py"
        deps = correlation_files+[script]
        output = './Derivatives/ln_ngal_%s.centralsonly.txt' % param
        targets = [output]
...
def task_compute_derivative_wp():
    ...
    for param_files, param in param_files_list:
        ...
        script = "./Analysis/compute_partial_derivative.py"
        deps = correlation_files+[script]
        output = './Derivatives/wp_%s.txt' % param
        targets = [output]
...
def task_compute_derivative_DeltaSigma():
    ...
    for param_files, param in param_files_list:
        ...
        script = "./Analysis/compute_partial_derivative.py"
        deps = correlation_files+[script]
        output = './Derivatives/DeltaSigma_%s.txt' % param
        targets = [output]
...
def task_compute_derivative_wp_centralsonly():
    ...
    for param_files, param in param_files_list_centralsonly:
        ...
        script = "./Analysis/compute_partial_derivative.py"
        deps = correlation_files+[script]
        output = './Derivatives/wp_%s.centralsonly.txt' % param
        targets = [output]
...
def task_compute_derivative_DeltaSigma_centralsonly():
    ...
    for param_files, param in param_files_list_centralsonly:
        ...
        script = "./Analysis/compute_partial_derivative.py"
        deps = correlation_files+[script]
        output = './Derivatives/DeltaSigma_%s.centralsonly.txt' % param
        targets = [output]
...
def task_compute_pk_gg_centralsonly():
    ...
    deps = [correlation_file, script]
    targets = ["./Covariances/pk_gg.centralsonly.txt"]
...
def task_compute_pk_gm_centralsonly():
    ...
    deps = [correlation_file, script]
    targets = ["./Covariances/pk_gm.centralsonly.txt"]
...
def task_compute_covariance_wp():
    ...
    pk_gg = './Covariances/pk_gg.txt'
    param_files = Path('./Covariances').glob('*.param')
...
def task_compute_covariance_DeltaSigma():
    ...
    pk_gg = './Covariances/pk_gg.txt'
    pk_gm = './Covariances/pk_gm.txt'
    pk_mm = './Covariances/pk_mm.txt'
    param_files = Path('./Covariances').glob('*.param')
...
def task_compute_covariance_wp_centralsonly():
    script = './Analysis/compute_covar_wp.py'
    pk_gg = './Covariances/pk_gg.centralsonly.txt'
    param_files = Path('./Covariances').glob('*.param')
...
def task_compute_covariance_DeltaSigma_centralsonly():
    script = './Analysis/compute_covar_DeltaSigma.py'
    pk_gg = './Covariances/pk_gg.centralsonly.txt'
    pk_gm = './Covariances/pk_gm.centralsonly.txt'
    pk_mm = './Covariances/pk_mm.txt'
    param_files = Path('./Covariances').glob('*.param')
...
def task_compute_fractional_covariance_wp():
    ...
    param_files = Path('./Covariances').glob('*.param_frac')
...
def task_compute_fisher_matrix_DeltaSigma():
    ...
    for param_file in param_files:
        ...
        derivative_files = [('./Derivatives/DeltaSigma_%s.txt' % param, param) for param_files, param in param_files_list]
        obs_files, params = zip(*derivative_files)
        input_string = ["-f %s %s" % file_tuple for file_tuple in derivative_files]

        fisher_matrix_file = "./Forecasts/" + param_file.name + ".DeltaSigma.fisher"
...
def task_compute_fisher_matrix_wp():
    ...
    for param_file in param_files:
        varied_params = get_varied_params_from_file(param_file)
        param_files_list = compute_param_files_list(varied_params,all_param_files)
        derivative_files = [('./Derivatives/wp_%s.txt' % param, param) for param_files, param in param_files_list]
        obs_files, params = zip(*derivative_files)
        input_string = ["-f %s %s" % file_tuple for file_tuple in derivative_files]

        fisher_matrix_file = "./Forecasts/" + param_file.name + ".wp.fisher"
...
def task_compute_fisher_matrix_ngal():
    ...
    for param_file in param_files:
        varied_params = get_varied_params_from_file(param_file)
        param_files_list = compute_param_files_list(varied_params,all_param_files)
        derivative_files = [('./Derivatives/ln_ngal_%s.txt' % param, param) for param_files, param in param_files_list]
        input_string = ["-f %s %s" % file_tuple for file_tuple in derivative_files]

        obs_files, params = zip(*derivative_files)
        fisher_matrix_file = "./Forecasts/" + param_file.name + ".ngal.fisher"
...
#        param_files_list = compute_param_files_list(varied_params,all_param_files_centralsonly)
        ...
        derivative_files = [('./Derivatives/DeltaSigma_%s.centralsonly.txt' % param, param) for param_files, param in param_files_list]
        obs_files, params = zip(*derivative_files)
        input_string = ["-f %s %s" % file_tuple for file_tuple in derivative_files]

        fisher_matrix_file = "./Forecasts/" + param_file.name + ".centralsonly.DeltaSigma.fisher"
...
#        param_files_list = compute_param_files_list(varied_params,all_param_files_centralsonly)
        param_files_list = param_files_list_centralsonly
        derivative_files = [('./Derivatives/wp_%s.centralsonly.txt' % param, param) for param_files, param in param_files_list]
        obs_files, params = zip(*derivative_files)
        input_string = ["-f %s %s" % file_tuple for file_tuple in derivative_files]

        fisher_matrix_file = "./Forecasts/" + param_file.name + ".centralsonly.wp.fisher"
...
def task_compute_fisher_matrix_ngal_centralsonly():
    ...
    for param_file in param_files:
        varied_params = get_varied_params_from_file(param_file)
        param_files_list = param_files_list_centralsonly 
        derivative_files = [('./Derivatives/ln_ngal_%s.centralsonly.txt' % param, param) for param_files, param in param_files_list]
        input_string = ["-f %s %s" % file_tuple for file_tuple in derivative_files]

        obs_files, params = zip(*derivative_files)
        fisher_matrix_file = "./Forecasts/" + param_file.name + ".centralsonly.ngal.fisher"
...
#         varied_params = get_varied_params_from_file(param_file)
#         param_files_list = compute_param_files_list(varied_params,all_param_files)
#         derivative_files = [('./Derivatives/ln_DeltaSigma_%s.txt' % param, param) for param_files, param in param_files_list]
#         obs_files, params = zip(*derivative_files)
#         input_string = ["-f %s %s" % file_tuple for file_tuple in derivative_files]

#         fisher_matrix_file = "./Forecasts/" + param_file.name + ".DeltaSigma.fisher_frac"
...
#         varied_params = get_varied_params_from_file(param_file)
#         param_files_list = compute_param_files_list(varied_params,all_param_files)
#         derivative_files = [('./Derivatives/ln_wp_%s.txt' % param, param) for param_files, param in param_files_list]
#         obs_files, params = zip(*derivative_files)
#         input_string = ["-f %s %s" % file_tuple for file_tuple in derivative_files]

#         fisher_matrix_file = "./Forecasts/" + param_file.name + ".wp.fisher_frac"
...
#         obs_files, params = zip(*derivative_files)
#         fisher_matrix_file = "./Forecasts/" + param_file.name + ".ngal.fisher_frac"
...
def task_add_fisher_matrices():
    ...
    for param_file in param_files:
        wp_fisher_matrix_file = "./Forecasts/" + param_file.name + ".wp.fisher"
        ds_fisher_matrix_file = "./Forecasts/" + param_file.name + ".DeltaSigma.fisher"
        ngal_fisher_matrix_file = "./Forecasts/" + param_file.name + ".ngal.fisher"
        output_fisher = './Forecasts/' + param_file.name + '.fisher'
        output_cov = './Forecasts/' + param_file.name + '.parameter_covariance'
...
def task_add_fisher_matrices_centralsonly():
    ...
    for param_file in param_files:
        wp_fisher_matrix_file = "./Forecasts/" + param_file.name + ".centralsonly.wp.fisher"
        ds_fisher_matrix_file = "./Forecasts/" + param_file.name + ".centralsonly.DeltaSigma.fisher"
        ngal_fisher_matrix_file = "./Forecasts/" + param_file.name + ".centralsonly.ngal.fisher"
        output_fisher = './Forecasts/' + param_file.name + '.centralsonly.fisher'
        output_cov = './Forecasts/' + param_file.name + '.centralsonly.parameter_covariance'
...
def task_add_fisher_matrices_DeltaSigma_ngal():
    ...
    for param_file in param_files:
        ds_fisher_matrix_file = "./Forecasts/" + param_file.name + ".DeltaSigma.fisher"
        ngal_fisher_matrix_file = "./Forecasts/" + param_file.name + ".ngal.fisher"
        output_fisher = './Forecasts/' + param_file.name + '.DS+ngal.fisher'
        output_cov = './Forecasts/' + param_file.name + '.DS+ngal.parameter_covariance'
...
def task_add_fisher_matrices_wp_ngal():
    ...
    for param_file in param_files:
        ...
        output_fisher = './Forecasts/' + param_file.name + '.wp+ngal.fisher'
        output_cov = './Forecasts/' + param_file.name + '.wp+ngal.parameter_covariance'
...
#     for param_file in param_files:
#         wp_fisher_matrix_file = "./Forecasts/" + param_file.name + ".wp.fisher_frac"
#         ds_fisher_matrix_file = "./Forecasts/" + param_file.name + ".DeltaSigma.fisher_frac"
#         ngal_fisher_matrix_file = "./Forecasts/" + param_file.name + ".ngal.fisher_frac"
#         output_fisher = './Forecasts/' + param_file.name + '.fisher_frac'
#         output_cov = './Forecasts/' + param_file.name + '.parameter_covariance_frac'
...
#         ds_fisher_matrix_file = "./Forecasts/" + param_file.name + ".DeltaSigma.fisher_frac"
#         ngal_fisher_matrix_file = "./Forecasts/" + param_file.name + ".ngal.fisher_frac"
#         output_fisher = './Forecasts/' + param_file.name + '.DS+ngal.fisher_frac'
#         output_cov = './Forecasts/' + param_file.name + '.DS+ngal.parameter_covariance_frac'
...
#         output_fisher = './Forecasts/' + param_file.name + '.wp+ngal.fisher_frac'
#         output_cov = './Forecasts/' + param_file.name + '.wp+ngal.parameter_covariance_frac'
...
def task_plot_ellipses():
    ...
    for param_file in param_files:
        cov_file = './Forecasts/' + param_file.name + '.parameter_covariance'
        deps = [cov_file, script]
        targets = ['Forecasts/' + param_file.name + '.ellipses.pdf']
...
def task_plot_ellipses_centralsonly():
    ...
    for param_file in param_files:
        cov_file = './Forecasts/' + param_file.name + '.centralsonly.parameter_covariance'
        deps = [cov_file, script]
        targets = ['Forecasts/' + param_file.name + '.centralsonly.ellipses.pdf']
...
#     for param_file in param_files:
#         cov_file = './Forecasts/' + param_file.name + '.parameter_covariance_frac'
#         deps = [cov_file, script]
#         targets = ['Forecasts/' + param_file.name + '.ellipses_frac.pdf']
...
def task_plot_ellipses_wp_ngal():
    ...
    for param_file in param_files:
        cov_file = './Forecasts/' + param_file.name + '.wp+ngal.parameter_covariance'
        deps = [cov_file, script]
        targets = ['Forecasts/' + param_file.name + '.wp+ngal.ellipses.pdf']
...
#     for param_file in param_files:
#         cov_file = './Forecasts/' + param_file.name + '.DS+ngal.parameter_covariance'
#         deps = [cov_file, script]
#         targets = ['Forecasts/' + param_file.name + '.DS+ngal.ellipses.pdf']
...
#     for param_file in param_files:
#         cov_file = './Forecasts/' + param_file.name + '.wp+ngal.parameter_covariance_frac'
#         deps = [cov_file, script]
#         targets = ['Forecasts/' + param_file.name + '.wp+ngal.ellipses_frac.pdf']
...
#     for param_file in param_files:
#         cov_file = './Forecasts/' + param_file.name + '.DS+ngal.parameter_covariance_frac'
#         deps = [cov_file, script]
